...
```bash
# 1. crie o bot com o @BotFather e envie uma mensagem a ele
$env:TELEGRAM_TOKEN = "seu_token"

# 2. descubra o chat_id
python configurar_telegram.py --descobrir
$env:TELEGRAM_CHAT_ID = "numero_encontrado"

# 3. valide
python configurar_telegram.py --testar
python configurar_telegram.py --simular   # exemplo de cada tipo de alerta
```

Com as duas variáveis definidas, o canal entra **automaticamente** no monitor e
no pipeline. Sem elas, o sistema roda igual usando console e arquivo — nenhuma
alteração de código é necessária nos dois casos.

Três decisões pensadas para operação em campo:

- **Falha de rede não perde alerta.** Se o envio falhar, a mensagem vai para uma
  fila e é reenviada na próxima tentativa. Uma enchente derruba conectividade
  justamente quando o alerta importa, e atrasar é melhor que perder.
- **Intervalo mínimo entre envios.** A histerese já evita repetição, mas uma
  falha em cascata não pode virar dezenas de notificações.
- **O monitor notifica na mudança de patamar de risco**, e não a cada ciclo.
  Rodando de hora em hora, repetir "situação normal" 24 vezes por dia seria
  ruído; o que interessa é a transição.
- **Alertas que persistem são repetidos periodicamente** (padrão: a cada 6 h,
  ajustável com `--lembrete`, ou `0` para desativar). Um alerta que dura doze
  horas sem nenhuma repetição corre o risco de ser esquecido, e quem responde
  precisa saber que a condição continua ativa. A mensagem de lembrete informa
  há quanto tempo a condição dura.
